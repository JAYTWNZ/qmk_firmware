...
const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
 [MAC_BASE] = LAYOUT_ansi_98(
    ...
    [WIN_BASE] = LAYOUT_ansi_98(
        ...
        KC_CAPS,  KC_A,     KC_S,     KC_D,     KC_F,     KC_G,     KC_H,     KC_J,     KC_K,     KC_L,     KC_SCLN,  KC_QUOT,            KC_ENT,             KC_P4,    KC_P5,    KC_P6,
        KC_LSFT,            KC_Z,     KC_X,     KC_C,     KC_V,     KC_B,     KC_N,     KC_M,     KC_COMM,  KC_DOT,   KC_SLSH,            KC_RSFT,  KC_UP,    KC_P1,    KC_P2,    KC_P3,      KC_PENT,
        KC_LCTL,  KC_LWIN,  KC_LALT,                                KC_SPC,                                 KC_RCMMD, FN_WIN,   KC_RCTL,  KC_LEFT,  KC_DOWN,  KC_RGHT,  KC_P0,    KC_PDOT            ),
...
// 【修正：音量旋鈕核心回歸原廠相容變數，並確保 encoder_map 的陣列結構完全吻合 keymaps 數量】
...
bool rgb_matrix_indicators_user(void) {
    ...
    // 平時預設：強制全鍵盤亮螢光綠色 (#35ff14 -> 53, 255, 20)
    for (uint8_t i = 0; i < RGB_MATRIX_LED_COUNT; i++) {
        rgb_matrix_set_color(i, 53, 255, 20);
    }
    
    // 【大寫鎖定連動】：由於流水號錯位，我們用一組精確測試出來的物理索引直接對準 26 個英文字母與 Caps 鍵
    if (host_keyboard_led_state().caps_lock) {
        ...
        for (uint8_t i = 0; i < 28; i++) {
            rgb_matrix_set_color(caps_leds[i], 255, 96, 0); // 變橘色 (#ff6000)
        }
    }

    // 【數字鎖定連動】：精確對準右側整片九宮格（NumLock、所有數字、Enter加長鍵）
    if (host_keyboard_led_state().num_lock) {
        uint8_t numpad_leds[] = {
            13, 14, 15, 16, 17, 28, 40, 41, 42, 43, 
            44, 58, 59, 60, 61, 75, 76, 77, 78, 79
        };
        for (uint8_t i = 0; i < 20; i++) {
            rgb_matrix_set_color(numpad_leds[i], 255, 96, 0); // 變橘色 (#ff6000)
        }
    }
